...
do
    ...
    command time -v --output=./logs/time_output_${HDRS_PER_STEP}_${NUM_STEPS}.txt ./target/release/examples/validate_btc_header_batch scripts/headers_100000.txt $HDRS_PER_STEP $NUM_STEPS > ./logs/output_${HDRS_PER_STEP}_${NUM_STEPS}.txt
    if [ $i -ne 7 ]; then
